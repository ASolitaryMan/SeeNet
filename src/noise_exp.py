...
import random
import librosa
# from balancedsampler import BalancedBatchSampler,BalancedBatchSamplerX
import warnings
...
class MERDataset(Dataset):
    ...
    def __init__(self, src_path, is_train=False, mix_prob=0.5, db=0, noise_db=""):
        ...
        self.random_list = {}
        self.tmp_random_list = [0,1,2,3]
        self.noise_list = []
        for line in all_lines:
            tmp = line.strip().split("\n")[0].split()
            self.wav_list.append(tmp[1])
            self.label.append(emo2idx[tmp[-1]])
            if emo2idx[tmp[-1]] not in self.random_list:
                self.random_list[emo2idx[tmp[-1]]] = [tmp[1]]
            else:
                self.random_list[emo2idx[tmp[-1]]].append(tmp[1])

        all_noise_line = read_text(noise_db)

        for item in all_noise_line:
            tmp = item.strip().split("\n")[0].split()
            self.noise_list.append(tmp[1])

        self.snr = AddGaussianSNR(min_snr_in_db=db, max_snr_in_db=db, p=1)
        self.db = db
    ...
    def __getitem__(self, index):
        ...
        if self.is_train == True:
            if random.random() < 0.0:
                ...
                cls_id = random.sample(self.tmp_random_list, 1)[0]
                sec_idx = random.randint(0, len(self.random_list[cls_id]) - 1)
                sec_wav, ssr = sf.read(self.random_list[cls_id][sec_idx])
                ...
                pass #[-5,5],从（0，L/2）先一个数l， 从（0，L-l）选一个数， 取出sec语音的语音段，计算两个语音的短时能量，按照公式混合两段语音
        else:
            idx = random.randint(0, len(self.noise_list)-1)
            noise_wave, sr = sf.read(self.noise_list[idx])

            assert sr==16000

            if len(noise_wave) > 6*sr:
                noise_wave = noise_wave[:6*sr]
            
            if self.db != 0:
                noise_wave = self.snr(noise_wave, sample_rate=16000) + noise_wave
            
            if len(wave) > len(noise_wave):
                wave[:len(noise_wave)] = wave[:len(noise_wave)] + noise_wave
            else:
                wave = wave + noise_wave[:len(wave)]
...
def get_loaders(args, train_path, valid_path, test_path):
    ...
    train_dataset = MERDataset(train_path, is_train=True, db=args.db, noise_db=args.noise_db)
    class_weight = train_dataset.class_weight_k()
    valid_dataset = MERDataset(valid_path, is_train=False, db=args.db, noise_db=args.noise_db)
    test_dataset = MERDataset(test_path, is_train=False, db=args.db, noise_db=args.noise_db)
...
class WavlmForClassification(WavLMPreTrainedModel):
    def __init__(self, config, pooling_mode="atten", num_class=4, num_aux=4):
...
def main(args):
    ...
    model = WavlmForClassification.from_pretrained(args.model_path, args.pooling_model, args.num_class, args.num_aux)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--num_aux', type=int, default=4, help='The numbers of emotion class')

    parser.add_argument('--loss_rate', type=float, default=0, help='multi-task loss rate')
    parser.add_argument('--db', type=int, default=20, help='the degree of SNR')
    ...
    parser.add_argument('--train_src', type=str, default="/home/lqf/workspace/icassp2023/session2_train.scp", help='the path of train src')
    parser.add_argument('--valid_src', type=str, default="/home/lqf/workspace/icassp2023/session2_valid.scp", help='the path of valid src')
    parser.add_argument('--test_src', type=str, default="/home/lqf/workspace/icassp2023/ses02M.scp", help='the path of test src')
    parser.add_argument('--noise_db', type=str, default="/home/lqf/workspace/wavlm-multi/de-noise.scp", help='the path of test src')
    #8,8
