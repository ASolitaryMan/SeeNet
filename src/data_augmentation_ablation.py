...
from audiomentations import AddGaussianSNR, TimeStretch, PitchShift
...
class MERDataset(Dataset):
    ...
    def __init__(self, src_path, is_train=False, mix_prob=0.5, aug_mtd=""):
        ...
        self.psh = PitchShift(min_semitones=-4, max_semitones=4,p=1)
        ...
        if self.is_train == True:
            if self.aug_mtd == "SNR":
                if random.random() < 0.5:
                    ...
                    wave = self.snr(wave, sr)

            if self.aug_mtd == "SPEED":
                if random.random() < 0.5:
                    pass
                else:
                    wave = self.timestretch(wave, sr)
            
            if self.aug_mtd == "PITCH":
                if random.random() < 0.5:
                    pass
                else:
                    wave = self.psh(wave, sr)
            ...
            if self.aug_mtd == "MIXUP":
                if random.random() < 0.5:
                    ...
                    if self.sec_len > len(wave):
                        wave = (1-r) * wave + r * sec_wav[:len(wave)]
                    else:
                        sec_wav = np.pad(sec_wav, (0, len(wave) - self.sec_len), mode='mean')
                        wave = (1-r) * wave + r * sec_wav
...
def get_loaders(args, train_path, valid_path, test_path, is_aug=False, aug_mtd="EMP"):
    ...
    train_dataset = MERDataset(train_path, is_train=is_aug, aug_mtd=aug_mtd)
    class_weight = train_dataset.class_weight_k()
    valid_dataset = MERDataset(valid_path, is_train=False)
    test_dataset = MERDataset(test_path, is_train=False)
    ...
    return train_dataloader, valid_dataloader, None, class_weight
...
class WavlmForClassification(WavLMPreTrainedModel):
    def __init__(self, config, pooling_mode="atten", num_class=4):
...
def train_model(accelerator, model, cls_loss, dataloader, lr_scheduler=None, optimizer=None, train=False, lrate=0.1):
    ...
    for data in dataloader:
        ...
        with accelerator.accumulate(model):
            optimizer.zero_grad()
            emos_out, _, _ = model(input_values=input_values, attention_mask=attention_mask, labels=emos)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--valid_src', type=str, default="/home/lqf/workspace/icassp2023/session3_valid.scp", help='the path of valid src')
    parser.add_argument('--test_src', type=str, default="/home/lqf/workspace/icassp2023/session3_test.scp", help='the path of test src')
    parser.add_argument('--loss_rate', type=float, default=0.01, help='multi-task loss rate')
    parser.add_argument('--is_augment', action='store_true', default=False, help='whether to do data augmentation')
    parser.add_argument('--method_augment', type=str, default="EMP", help='The method to do data augmentation')
    #8,8
    ...
    train_loader, eval_loader, test_loader, class_weight = get_loaders(args, train_src_path, valid_src_path, test_src_path, is_aug=args.is_augment, aug_mtd=args.method_augment)  
    ...
    model = WavlmForClassification.from_pretrained(args.model_path, pooling_mode="atten", aux_task=args.aux_task, num_c=args.num_class_c)
