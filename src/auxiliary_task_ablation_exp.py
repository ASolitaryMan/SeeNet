...
from audiomentations import AddGaussianSNR, TimeStretch
...
class DataCollatorCTCWithPadding:
    ...
    def __call__(self, features: List) -> Dict[str, torch.Tensor]:
        input_features, emo_label, aux_lab = [], [], []
        ...
        d_type = torch.long if isinstance(aux_lab[0], int) else torch.float32
        batch["aux_labels"] = torch.tensor(aux_lab, dtype=d_type)
...
class MERDataset(Dataset):
    ...
    def __init__(self, src_path, mix_prob=0.5, aux_task="SPEAKER"):
        ...
        for line in all_lines:
            ...
            self.spk_list.append(spk)
            if spk not in self.spk_dict:
                self.spk_dict[spk] = len(self.spk_dict)
            self.wav_list.append(tmp[3])
            self.label.append(emo2idx[tmp[-1]])
            self.gen_list.append(gen)
        self.len = self.__len__()
        self.snr = AddGaussianSNR(min_snr_in_db=0, max_snr_in_db=20, p=1)
        self.timestretch = TimeStretch(min_rate=0.9, max_rate=1.1, p=1)
    ...
    def __getitem__(self, index):
        ...
        if self.aux_task == "SPEAKER":
            spk = self.spk_list[index]
            auxiliary_label = self.spk_dict[spk]

        if self.aux_task == "GENDER":
            gen = self.gen_list[index]
            if gen == "F":
                auxiliary_label = 0
            else:
                auxiliary_label = 1

        if self.aux_task == "SEE":
            auxiliary_label = 0
    
        return torch.FloatTensor(wave), lab, auxiliary_label
...
def get_loaders(args, train_path, valid_path, test_path, aux_task="SPEAKER"):
    feature_extractor = Wav2Vec2FeatureExtractor.from_pretrained(args.save_root, return_attention_mask=True)
    data_collator = DataCollatorCTCWithPadding(feature_extractor=feature_extractor, padding=True, max_length=args.max_length)

    train_dataset = MERDataset(train_path, aux_task=aux_task)
    class_weight = train_dataset.class_weight_k()
    valid_dataset = MERDataset(valid_path, is_train=False, aux_task=aux_task)
...
class WavlmForClassification(WavLMPreTrainedModel):
    def __init__(self, config, pooling_mode="atten", aux_task="GENDER", num_class=4, num_aux=4):
        ...
        self.aux_task = aux_task
        self.num_a = num_aux
        assert self.aux_task in ["SEE", "GENDER", "SPEAKER"]
        ...
        if self.aux_task == "GENDER":
            ...
            self.aux_classifier = HubertClassificationHead(num_class=self.num_a)
        ...
        self.classifier = HubertClassificationHead(num_class=num_class)
    ...
    def forward(
        ...
        if self.aux_task == "SEE":
            ...
            emos_out = self.classifier(hidden_states) #四分类

            return emos_out, experts_emb, experts_lab

        if self.aux_task != None:
            auxs_out = self.aux_classifier(hidden_states)

        emos_out = self.classifier(hidden_states)

        return emos_out, auxs_out, torch.zeros(emos_out.size()[0])      
...
def train_model(accelerator, model, cls_loss, dataloader, lr_scheduler=None, optimizer=None, train=False, lrate=0.1, aux_task="SPEAKER"):
    ...
    for data in dataloader:
        ## analyze dataloader
        input_values, attention_mask, emos, auxs = data["input_values"], data["attention_mask"], data["emo_labels"], data['aux_labels']
        ...
        with accelerator.accumulate(model):
            ...
            if aux_task == "SEE":
                ...
                loss = cls_loss(emos_out, emos) + lrate * cls_loss(experts_emb, auxs)
...
def eval_model_multi(accelerator, model, cls_loss, dataloader, optimizer=None, train=False, lrate=0.1):
    ...
    emo_probs = lrate * emo_probs + (1-lrate) * emos_exp_prebs #反过来的
    emo_preds = np.argmax(emo_probs, 1)
    # emo_accuracy = accuracy_score(emo_labels, emo_preds)
    emo_ua, emo_accuracy = unweightedacc(emo_labels, emo_preds)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--num_class', type=int, default=10, help='num_class')
    parser.add_argument('--num_aux', type=int, default=4, help='num_unit for auxiliary task')
    ...
    parser.add_argument('--aux_task', type=str, default="SPEAKER", help='The kind of auxiliary task')
    #8,8
    ...
    assert args.aux_task in ["SPEAKER","SEE","GENDER"] 
    ...
    train_loader, eval_loader, test_loader, class_weight = get_loaders(args, train_src_path, valid_src_path, test_src_path, aux_task=args.aux_task)  
    ...
    model = WavlmForClassification.from_pretrained(args.model_path, pooling_mode="atten", aux_task=args.aux_task, num_class=args.num_class, num_aux=args.num_aux)
    ...
    for epoch in range(args.epochs):
        ...
        train_results = train_model(accelerator, model, cls_loss, train_loader, lr_scheduler=None, optimizer=optimizer, train=True, lrate=loss_rate, aux_task=args.aux_task)
        if args.aux_task != "SEE":
